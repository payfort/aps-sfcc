...
const Checkout = () => {
    ...
    const submitOrderApplePay = async () => {
        submitOrder()
    }
    ...
    const submitOrder = async () => {
        ...
        try {
            ...
            if (order?.c_paymentRedirectUrl) {
                window.sessionStorage.setItem('orderNo', order.orderNo)

                window.location.replace(order.c_paymentRedirectUrl)
                return
            }

            const orderPaymentInstrument = order?.paymentInstruments[0]
            if (orderPaymentInstrument && orderPaymentInstrument?.paymentMethodId === PAYMENT_METHODS_IDS.APS_HOSTED && orderPaymentInstrument?.c_apsPaymentData) {
                const apsHostedData = JSON.parse(orderPaymentInstrument.c_apsPaymentData)
                const formContainer = document.getElementById('aps-hosted-form')
                // Loop through the keys of the object
                for (const key in apsHostedData) {
                    if (apsHostedData.hasOwnProperty(key)) {
                        // Create input element
                        const inputField = document.createElement('input')
                        inputField.setAttribute('type', 'hidden')
                        inputField.setAttribute('name', key)
                        inputField.setAttribute('value', apsHostedData[key])
                ...
                <Grid templateColumns={{base: '1fr', lg: '66% 1fr'}} gap={{base: 10, xl: 20}}>
                    <GridItem>
                        <Stack spacing={4}>
                            ...
                            <ShippingAddress />
                            <ShippingOptions />
                            <Payment submitOrderApplePay={submitOrderApplePay} />

...
const CreditCardFields = ({form, prefix = '', isAccount}) => {
    // OVERRIDE START
    ...
    return (
        <Box>
            <Stack spacing={5}>
                ...
                {/** OVERRIDE START */}
                {isRegistered && !isAccount && (
                    <Field
                        {...fields.saveCard}
                        formLabel={<FormLabel marginBottom={0}>{fields.saveCard.label}</FormLabel>}
                    />
                )}
                {/** OVERRIDE END */}
            </Stack>
...
CreditCardFields.propTypes = {
    ...
    /** Indicates if the form is rendered in customer account page */
